    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 font-medium">
          รายชื่อและขีดความสามารถของเจ้าหน้าที่ปฏิบัติงานซ่อม คณะสังคมศาสตร์
        </p>
      </div>
        ...
        {staff.map((person) => (
          ...
          >
            ...
            <div className="relative">
              <img 
                src={person.imageUrl || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=150&auto=format&fit=crop&q=80'} 
                alt={person.name} 
            ...
            <div className="mt-4">
              ...
              <p className="text-xs text-primary dark:text-red-400 font-bold mt-1.5 bg-red-100/30 dark:bg-red-950/20 px-2.5 py-1 rounded-full">
                {person.role}
              </p>
            ...
            <div className="mt-5 w-full space-y-1.5 text-left bg-slate-50/50 dark:bg-slate-800/10 p-3 rounded-xl border border-slate-50 dark:border-slate-800 text-[10px] text-slate-400 font-semibold">
              <div className="flex items-center gap-1.5">
                ...
                <span>เบอร์สายด่วน: 081-123-456{person.id.slice(-1)}</span>
              </div>
              <div className="flex items-center gap-1.5">
                <Mail className="w-3.5 h-3.5 text-slate-400" />
                <span className="font-mono">email: repair.staff.{person.id.toLowerCase()}@ku.th</span>
              </div>
            </div>
          </div>
        ))}
